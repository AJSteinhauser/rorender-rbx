...
interface MaterialMenuProps {
	...
	onMaterialSelected?: (material: Enum.Material) => void;
}
...
export function MaterialMenu(props: MaterialMenuProps) {
	const onMaterialSelected = props.onMaterialSelected || function () {};
	...
	if (window && opened) {
			...
			items[material.Name] = (
				<Button
                    ...
                    visible={searchfilter.map((text: string) => {
                        return findString(text, material.Name)
                    })}
				/>
			);
		...
		const numVisibleRows = math.min(
			Enum.Material.GetEnumItems().size(),
			maxVisibleRows
		);
	...
	return (
		<imagebutton
			Size={props.Size}
			Image={'rbxasset://textures/MaterialFramework/Dark/Material.png'}
