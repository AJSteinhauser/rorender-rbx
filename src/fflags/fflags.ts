...
export const getFFlags = (): FeatureFlagSchema => {
    ...
    if (usingLocalFFlags) {
        ...
        try {
            ...
            if (!getFlags) {
                warn(
                    "Local fflag has not been modified from testing state, reverting to prod fflags"
                )
                fflags = localFeatureFlags
            } else {
                fflags = getFlags
            ...
            fflags = localFeatureFlags
            warn(
                "There was a problem retrieving hosted fflags, reverting to local flags"
            )
            print(e)
        }
    }
...
const getHostedSettings = (): FeatureFlagSchema | undefined => {
    ...
    //}
    const response = httpService.GetAsync(
        "https://raw.githubusercontent.com/AJSteinhauser/rorender-rbx/refs/heads/main/fflags.json",
        true
    )
