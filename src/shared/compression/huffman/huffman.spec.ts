...
const checkTreesAreEqual = (
    ...
    if (!tree1 || !tree2) {
        // Only for typechecker to shutup
        return false
    }
    if (
        ...
        return (
            checkTreesAreEqual(tree1.left, tree2.left) &&
            checkTreesAreEqual(tree1.right, tree2.right)
        )
    }
}
...
export = () => {
    describe("tree storage", () => {
        const frequencyTable = generatePriorityQueue(
            buffer.fromstring(TEST_STRING)
        )
    ...
    describe("frequency table", () => {
        ...
        it("should build a frequncy table that matches expected output", () => {
            generatePriorityQueue(buffer.fromstring(TEST_STRING)).forEach(
                (node, index) => {
                    expect(node.frequency).to.equal(
                        expectedFrequencyTable[index].frequency
                    )
                    expect(node.symbol).to.equal(
                        expectedFrequencyTable[index].symbol
                    )
                }
            )
        })
        ...
        it("should build a priority queue with a length equal to the number of symbols", () => {
            expect(
                generatePriorityQueue(buffer.fromstring(TEST_STRING)).size()
            ).to.equal(4)
        })
    })
    ...
    describe("huffman tree", () => {
        ...
        it("should build a huffman tree that matches expected output", () => {
            expect(
                checkTreesAreEqual(expectedHuffmanTree, huffmanTree)
            ).to.equal(true)
        })
    })
    ...
    describe("huffman table", () => {
        ...
        expectedHuffmanTable.set(getCharByteValue("A"), {
            binaryValue: 0b1,
            bitLength: 1
        }) // 0b1)
        expectedHuffmanTable.set(getCharByteValue("B"), {
            binaryValue: 0b00,
            bitLength: 2
        }) // 0b1)
        expectedHuffmanTable.set(getCharByteValue("C"), {
            binaryValue: 0b010,
            bitLength: 3
        }) // 0b1)
        expectedHuffmanTable.set(getCharByteValue("D"), {
            binaryValue: 0b011,
            bitLength: 3
        }) // 0b1)
        ...
        it("should encode the huffmen codes to the table", () => {
            huffmanTable.forEach((value, key) => {
                expect(value.binaryValue).to.equal(
                    expectedHuffmanTable.get(key)?.binaryValue
                )
                expect(value.bitLength).to.equal(
                    expectedHuffmanTable.get(key)?.bitLength
                )
            })
        })
    })
    ...
    describe("huffman encoding", () => {
        ...
        huffmanTable.set(getCharByteValue("A"), {
            binaryValue: 0b1,
            bitLength: 1
        }) // 0b1)
        huffmanTable.set(getCharByteValue("B"), {
            binaryValue: 0b00,
            bitLength: 2
        }) // 0b1)
        huffmanTable.set(getCharByteValue("C"), {
            binaryValue: 0b010,
            bitLength: 3
        }) // 0b1)
        huffmanTable.set(getCharByteValue("D"), {
            binaryValue: 0b011,
            bitLength: 3
        }) // 0b1)
        ...
        it("should encode a single character", () => {
            ...
            expect(buffer.readu32(encoded.data, 0)).to.equal(
                0b10000000000000000000000000000000
            )
            const test2 = huffmanEncode(buffer.fromstring("C"), huffmanTable)
            expect(buffer.readu32(test2.data, 0)).to.equal(
                0b01000000000000000000000000000000
            )
        })

        it("should encode multiple characters", () => {
            const encoded = huffmanEncode(
                buffer.fromstring("ABA"),
                huffmanTable
            )
            expect(buffer.readu32(encoded.data, 0)).to.equal(
        ...
        it("should encode the test string", () => {
            ...
            expect(buffer.readu32(encoded.data, 0)).to.equal(
                0b11000000000011100010010110110000
            )
        })
        ...
        it("should encode 3 bit repeating char overflowing string", () => {
            ...
            expect(buffer.readu32(encoded.data, 4)).to.equal(
    ...
    describe("huffman decoding", () => {
        ...
        it("should decode a single character", () => {
            const test1Encoded = huffmanEncode(
                buffer.fromstring("A"),
                huffmanTable
            )
            const test1Decoded = huffmanDecode(
                test1Encoded.data,
                test1Encoded.bitLength,
                huffmanTree
            )
            expect(buffer.readstring(test1Decoded, 0, 1)).to.equal("A")
            const test2Encoded = huffmanEncode(
                buffer.fromstring("B"),
                huffmanTable
            )
            const test2Decoded = huffmanDecode(
                test2Encoded.data,
                test2Encoded.bitLength,
                huffmanTree
            )
            expect(buffer.readstring(test2Decoded, 0, 1)).to.equal("B")
            const test3Encoded = huffmanEncode(
                buffer.fromstring("C"),
                huffmanTable
            )
            const test3Decoded = huffmanDecode(
                test3Encoded.data,
                test3Encoded.bitLength,
                huffmanTree
            )
        ...
        it("should decode the test string", () => {
            const testEncoded = huffmanEncode(
                buffer.fromstring(TEST_STRING),
                huffmanTable
            )
            const testDecoded = huffmanDecode(
                testEncoded.data,
                testEncoded.bitLength,
                huffmanTree
            )
            expect(
                buffer.readstring(testDecoded, 0, buffer.len(testDecoded))
        ...
        it("should decode a long string with 1 bit encoding and buffer overflow", () => {
            ...
            const decoded = buffer.readstring(
                testDecoded,
                0,
                buffer.len(testDecoded)
            )
