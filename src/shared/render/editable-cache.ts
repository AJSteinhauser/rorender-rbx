...
type EditableCacheStore<T extends EditableClass> = Map<
    string,
    EditableCacheEntity<T>
>

const meshCacheStore: EditableCacheStore<EditableMesh> = new Map<
    string,
    EditableCacheEntity<EditableMesh>
>()
const imageCacheStore: EditableCacheStore<EditableImage> = new Map<
    string,
    EditableCacheEntity<EditableImage>
>()
...
export function getEditableImage(imageId: string): EditableImage {
    return getEditableFromId(
        imageId,
        imageCacheStore,
        getEditableImageFromRoblox
    )
}
...
function getEditableFromId<T extends EditableClass>(
    ...
    if (!editableCache || assetDoesNotExistsAndRetryLimitNotHit) {
        const creationAttempts =
            (editableCache && editableCache.creationAttempts) || 0
        try {
